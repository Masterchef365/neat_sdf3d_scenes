...
void main() {
    ...
    vec3 light = vec3(4. * sin(anim * 3.), 5. * cos(anim), 6. * cos(anim * 2.));
    const vec3 ambient = vec3(.1, .2, .3) * .3;
    vec3 BACKGROUND = vec3(0.); // Backgroudn color
    if (dot(unit_ray, normalize(light)) > 8.1) {
        BACKGROUND = vec3(1.);
    }
    ...
    for (i = 0; i < MAX_STEPS; i++) {
        ...
        if (hit.dist < HIT_THRESHOLD) {
            ...
            color = ambient + 
                hit.color * dot(lm, hit.normal) +
                vec3(0.1) * pow(max(dot(rf, unit_ray), 0.), 190.) * 5.;
            break;
        }
