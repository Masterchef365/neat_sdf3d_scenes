...
SDF scene(vec3 pos) {
    vec2 horz_space = pos.xz / 4.;
    float seed = rand(ivec2(horz_space));
    pos.xz = mix(vec2(-2.), vec2(2.), fract(horz_space));
    //float sep = 5. * (cos(seed * 3.14 + anim) + 1.) / 2.;
    float sep = 5. * (cos(anim + seed * 3.) + 1.) / 2.;
    return sdf_smooth_min(
    //return sdf_min(
        sphere(pos, vec3(0., sep, 0.0), vec3(0.721,seed,0.123), 1.0),
        sphere(pos, vec3(0., 0, 0.0), vec3(0.995,0.467,0.002), 1.0)
    );
}

const float CLIP_NEAR = 0.01; // Near clipping sphere
...
const vec3 BACKGROUND = vec3(1.); // Backgroudn color
